(function (global, factory) {
  ...
  **/
  ...
  var createCompiler = createCompilerCreator(function baseCompile (
    ...
    var ast = parse(template.trim(), options);  // 将template解析为ast树
    if (options.optimize !== false) {
      optimize(ast, options);   // 对ast优化
    }
    var code = generate(ast, options);  // 根据ast生成render、staticRenderFns
    return {
      ast: ast,
      render: code.render,
  ...
  var compileToFunctions = ref.compileToFunctions;  // 返回高阶函数的执行
